...
for i, line in enumerate(lines):
    ...
    if 'preds = np.clip(load_model().predict(X_test_ts), 0, None)' in line and 'preds_export' not in line:
    ...
    # Tab 5 export
    if 'preds_export = np.clip(load_model().predict(X_test_ts), 0, None)' in line:
